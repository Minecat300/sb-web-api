...
export async function addNewEmployee(employee) {
    ...
    try {
        ...
        const [result] = await conn.query(
            `INSERT INTO EMPLOYEES (first_name, last_name, display_name, username, email, start_date, department, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
export async function getAllNewEmployees() {
    ...
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.query(`SELECT * FROM EMPLOYEES WHERE status = 'pending'`);
        return rows;
    } catch (error) {
        throw error;
    } finally {
...
export async function activateNewEmployee(id) {
    ...
    const conn = await pool.getConnection();
    try {
        await conn.query(`UPDATE EMPLOYEES SET status = 'active' WHERE id = ?`, [id]);
    } catch (error) {
        throw error;
    } finally {
        conn.release();
...
export async function updateNewEmployee(id, employee) {
    ...
    try {
        const { first_name, last_name, display_name, username, email, start_date, department } = employee;
        await conn.query(
            `UPDATE EMPLOYEES SET first_name = ?, last_name = ?, display_name = ?, username = ?, email = ?, start_date = ?, department = ? WHERE id = ?`,
...
export async function getNewEmployeeByStatus(status) {
    ...
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.query(`SELECT * FROM EMPLOYEES WHERE status = ?`, [status]);
        return rows;
    } catch (error) {
        throw error;
    } finally {
...
export async function getNewEmployeeByUsername(username) {
    ...
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.query(`SELECT * FROM EMPLOYEES WHERE username = ?`, [username]);
        return rows[0];
    } catch (error) {
        throw error;
    } finally {
